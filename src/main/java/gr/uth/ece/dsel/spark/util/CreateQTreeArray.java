...
public class CreateQTreeArray
{
	...
	public static final void makeQuadArray(Point[] samplePoints, Node node)
	{
		...
		Arrays.sort(samplePoints, i1, i2 + 1, new PointXComparator("min")); // sort subarray [i1, i2] by x-ascending
		
		int j2 = xBinarySearchArray(samplePoints, i1, i2, (xmin + xmax) / 2); // get index of middle x
		
		Arrays.sort(samplePoints, i1, j2, new PointYComparator("min")); // sort subarray [i1, j2] by y-ascending
		
		int j1 = yBinarySearchArray(samplePoints, i1, j2, (ymin + ymax) / 2); // get index of middle y

		Arrays.sort(samplePoints, j2, i2 + 1, new PointYComparator("min")); // sort subarray [j2, i2] by y-ascending
