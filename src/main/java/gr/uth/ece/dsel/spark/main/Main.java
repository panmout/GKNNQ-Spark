...
{	
	public static void main(String[] args)
	{
		...
		SparkConf sparkConf = new SparkConf().setAppName("gknn-spark").setMaster(master);
	    ...
	    if (partitioning.equals("gd"))
	    	tPointsPerCellRDD = tpointsRDD
		    					.mapToPair(new PointToTupleCellOneGD(N))
		    					.reduceByKey(Integer::sum, partitions)
		    					.persist(StorageLevel.MEMORY_AND_DISK());
	    else if (partitioning.equals("qt"))
	    	tPointsPerCellRDD = tpointsRDD
								.mapToPair(new PointToTupleCellOneQT(root))
								.reduceByKey(Integer::sum, partitions)
		    					.persist(StorageLevel.MEMORY_AND_DISK());
		...
		final GetOverlaps getOverlaps = new GetOverlaps(K, mbrCentroidBC.getValue(), cell_tpoints, phase15);
	    ...
	    // ***** groupByKey method *****
	    //
	    // create RDD[cell, tpoints] from overlaps
	    JavaPairRDD<String, Iterable<Point>> overlapsCellTpointsRDD = JavaPairRDD.fromJavaRDD(jsc.emptyRDD());
	    
	    // filter tpoints RDD with overlapping cells only and convert it to RDD[cell, iterable<point>]
	    if (partitioning.equals("gd"))
	    	overlapsCellTpointsRDD = tpointsRDD.mapToPair(new PointToTupleCellPointGD(N))
					  						   .filter(new OverlapsContainCell(overlapsBC.getValue()))
	    								       .groupByKey(partitions)
	    								       .persist(StorageLevel.MEMORY_AND_DISK());
	    else if (partitioning.equals("qt"))
	    	overlapsCellTpointsRDD = tpointsRDD.mapToPair(new PointToTupleCellPointQT(root))
					  						   .filter(new OverlapsContainCell(overlapsBC.getValue()))
	    								       .groupByKey(partitions)
										       .persist(StorageLevel.MEMORY_AND_DISK());
	    ...
	    //
	    // ***** end groupByKey method *****
	    
	    // ***** aggregateByKey method *****
	    /*
	    if (partitioning.equals("gd"))
	    	neighbors2RDD = tpointsRDD.mapToPair(new PointToTupleCellPointGD(N))
					  				  .filter(new OverlapsContainCell(overlapsBC.getValue()))
				   					  .aggregateByKey(neighbors2, new BfNeighbors2(K, mbrCentroidBC.getValue(), qpointsBC.getValue(), neighbors2, fastsums, dpc_count), new MergePQ(K))
				   					  .values();
	    else if (partitioning.equals("qt"))
	    	neighbors2RDD = tpointsRDD.mapToPair(new PointToTupleCellPointQT(root))
					  				  .filter(new OverlapsContainCell(overlapsBC.getValue()))
				  					  .aggregateByKey(neighbors2, new BfNeighbors2(K, mbrCentroidBC.getValue(), qpointsBC.getValue(), neighbors2, fastsums, dpc_count), new MergePQ(K))
				  					  .values();
	    */
	    // ***** end aggregateByKey method *****
	    ...
	    */
	    ...
	    // ***** groupByKey method *****
	    //
	    // create RDD[cell, tpoints] from non-overlaps
	    JavaPairRDD<String, Iterable<Point>> nonOverlapsCellTpointsRDD = JavaPairRDD.fromJavaRDD(jsc.emptyRDD());
	    
	    // filter tpoints RDD with overlapping cells only and convert it to RDD[cell, iterable<point>]
	    if (partitioning.equals("gd"))
	    	nonOverlapsCellTpointsRDD = tpointsRDD.mapToPair(new PointToTupleCellPointGD(N))
					  							  .filter(new TpointsInNonPrunedCells(nonPrunedCellsBC.getValue()))
	    								       	  .groupByKey(partitions)
	    								       	  .persist(StorageLevel.MEMORY_AND_DISK());
	    else if (partitioning.equals("qt"))
	    	nonOverlapsCellTpointsRDD = tpointsRDD.mapToPair(new PointToTupleCellPointQT(root))
					  							  .filter(new TpointsInNonPrunedCells(nonPrunedCellsBC.getValue()))
	    								       	  .groupByKey(partitions)
										       	  .persist(StorageLevel.MEMORY_AND_DISK());
	    ...
	    /*
	    if (partitioning.equals("gd"))
		    neighbors3RDD = tpointsRDD.mapToPair(new PointToTupleCellPointGD(N))
					  				  .filter(new TpointsInNonPrunedCells(nonPrunedCellsBC.getValue()))
					   				  .aggregateByKey(neighbors3, new BfNeighbors2(K, mbrCentroidBC.getValue(), qpointsBC.getValue(), neighbors2, fastsums, dpc_count), new MergePQ(K))
					   				  .values();
	    else if (partitioning.equals("qt"))
	    	neighbors3RDD = tpointsRDD.mapToPair(new PointToTupleCellPointQT(root))
					  				  .filter(new TpointsInNonPrunedCells(nonPrunedCellsBC.getValue()))
									  .aggregateByKey(neighbors3, new BfNeighbors2(K, mbrCentroidBC.getValue(), qpointsBC.getValue(), neighbors2, fastsums, dpc_count), new MergePQ(K))
									  .values();
	    */
